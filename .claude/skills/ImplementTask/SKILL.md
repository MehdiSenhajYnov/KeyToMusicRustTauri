...
2. **For each existing doc file**, check if it references anything that changed:
   ...
   - **`KeyToMusic_Technical_Specification.md`** — Check: data model (Rust structs, TS types), algorithm pseudocode, architecture diagrams, command signatures, UI component descriptions, dependency versions. Update anything that's now stale.
...
- `KeyToMusic_Technical_Specification.md` — [sections updated, or "no changes needed"]
